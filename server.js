...
app.post('/api/coop/share-mission', (req, res) => {
  const { uuid, missionData } = req.body;
  if (!uuid || !missionData) return res.status(400).json({ error: 'uuid, missionData required' });
  db.shareMission(uuid, missionData);
  res.json({ success: true });
});

app.get('/api/coop/get-mission', (req, res) => {
  const { uuid, friend } = req.query;
  if (!uuid || !friend) return res.status(400).json({ error: 'uuid, friend required' });
  if (!db.areFriends(uuid, friend)) return res.status(400).json({ error: 'not friends' });
  const coopData = db.getCoopMission(friend);
  if (!coopData || !coopData.missionData) return res.json({ missionData: null, locks: {} });
  res.json({ missionData: coopData.missionData, locks: coopData.locks || {} });
});

app.post('/api/coop/lock-battle', (req, res) => {
  const { uuid, friend, eventId } = req.body;
  if (!uuid || !friend || !eventId) return res.status(400).json({ error: 'uuid, friend, eventId required' });
  if (!db.areFriends(uuid, friend)) return res.status(400).json({ error: 'not friends' });
  const profile = db.getProfile(uuid);
  const name = profile ? profile.name : 'Agent';
  const ok = db.lockBattle(friend, eventId, uuid, name);
  res.json({ success: ok });
});

app.post('/api/coop/unlock-battle', (req, res) => {
  const { uuid, friend, eventId } = req.body;
  if (!uuid || !friend || !eventId) return res.status(400).json({ error: 'uuid, friend, eventId required' });
  const ok = db.unlockBattle(friend, eventId, uuid);
  res.json({ success: ok });
});

app.post('/api/coop/complete-battle', (req, res) => {
  const { uuid, friend, eventId, score } = req.body;
  if (!uuid || !friend || !eventId || !score) return res.status(400).json({ error: 'uuid, friend, eventId, score required' });
  const result = db.completeBattle(friend, eventId, uuid, score);
  res.json(result);
});

app.get('/api/coop/battle-status', (req, res) => {
  const { friend } = req.query;
  if (!friend) return res.status(400).json({ error: 'friend required' });
  const locks = db.getBattleStatus(friend);
  res.json({ locks });
});
...
app.get('/api/download/:filename', (req, res) => {
  ...
  // If not found locally, redirect to Google Drive
  try {
    const versionFile = path.join(__dirname, 'version.json');
    let raw = fs.readFileSync(versionFile, 'utf8');
    raw = raw.replace(/[\x00-\x1F\x7F]/g, ' ').replace(/\s+/g, ' ').trim();
    ...
    if (data.version) {
      return res.redirect('https://github.com/Alpha777omg/maa-community-server/releases/download/v' + data.version + '/' + req.params.filename);
    }
  } catch (e) {}
